...
break;
	...
	for (int i = 0; i < m_vertices.size(); i++) {
		...
		//Iterate through array of unique coords
		for (int j = 0; j < uniques.size(); j++)
		{
			//Check if vertex coord is in array of unique coords
			if (m_vertices[i].getCoord() == uniques[j]) {
				duplicate = true;
				//Exit loop
				j = uniques.size();
			}
		...
		//If flagged as duplicate
		if (duplicate) {
			//erase element from m_vertices;
			m_vertices.erase(std::begin(m_vertices) + i);
			m_vertices.shrink_to_fit();
			//decrement i to account for shift in vector
			i--;
		}
		//Else vertex is unique so far
		else {
			//Add to collection of unique vertcies
			uniques.push_back(m_vertices[i].getCoord());
		}
	}
